...
ipcMain.handle('process-video', async (event, videoPath, bellTimestamps) => {
  const resultsDir = path.join(app.getPath('documents'), 'RTracker', 'Results', 
  ...
  for (let i = 0; i < bellTimestamps.length - 1; i++) {
    ...
    const outputPath = path.join(resultsDir, `segment_${i + 1}${path.extname(videoPath)}`);
...
ipcMain.handle('save-segment', async (event, segmentPath, saveAs) => {
  try {
    const result = await dialog.showSaveDialog(mainWindow, {
      ...
      filters: [{ name: 'Videos', extensions: [path.extname(segmentPath).substring(1)] }]
    });
