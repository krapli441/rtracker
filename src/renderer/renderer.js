...
function addOptimizeButton() {
  ...
  document.getElementById('optimizeButton').addEventListener('click', async () => {
    ...
    try {
      ...
      // 오디오 분석기의 최적화 함수 호출
      const optimizedOptions = audioAnalyzer.optimizeDetectionSettings(waveformData);
      
      // UI 업데이트
      amplitudeThresholdInput.value = optimizedOptions.amplitudeThreshold.toFixed(2);
      amplitudeThresholdValue.textContent = amplitudeThresholdInput.value;
      ...
      // 안내 메시지
      alert(`설정이 자동 최적화되었습니다.\n임계값: ${optimizedOptions.amplitudeThreshold.toFixed(2)}\n최소 간격: ${optimizedOptions.minBellInterval}초`);
        ...
        if (bellTimestamps.length === 0) {
          if (debugInfo && debugInfo.candidateBells.length > 0) {
            ...
            const suggestedThreshold = Math.max(
              0.05,
              debugInfo.candidateBells.reduce((min, b) => Math.min(min, b.peakAmplitude), 1) * 0.9
            ).toFixed(2);
            
            const result = confirm(`벨 소리가 감지되지 않았습니다. 후보 벨 소리가 ${debugInfo.candidateBells.length}개 있지만 조건에 맞지 않아 제외되었습니다.\n\n임계값을 ${suggestedThreshold}로 낮추고 다시 시도할까요?`);
...
function displayWaveformMarkers(timestamps, debug = null) {
  ...
  if (totalDuration > 0) {
    ...
    // 재생 위치 표시기
    markersHTML += '<div id="playerPositionMarker" class="player-position-marker"></div>';
  }
  ...
  if (debug) {
    ...
    if (debug.candidateBells && debug.candidateBells.length > 0) {
      debug.candidateBells.forEach(bell => {
        ...
        if (!debug.acceptedBells.find(b => Math.abs(b.start - bell.start) < 0.1)) {
          const position = (bell.start / totalDuration) * 100;
          markersHTML += `<div class="bell-candidate" style="left: ${position}%" 
            data-time="${bell.start}" 
            title="후보 벨: ${formatTime(bell.start)}, 진폭: ${bell.peakAmplitude.toFixed(2)}"></div>`;
        }
      });
    }
  ...
  if (timestamps.length === 0) {
    ...
    if (debug && debug.candidateBells && debug.candidateBells.length > 0) {
      ...
      const suggestedThreshold = Math.max(
        0.05,
        debug.candidateBells.reduce((min, b) => Math.min(min, b.peakAmplitude), 1) * 0.9
      ).toFixed(2);
      
      markersHTML += `<div class="debug-info">
        <h4>문제 해결 정보</h4>
        <p>${debug.candidateBells.length}개의 벨 소리 후보가 있지만 조건에 맞지 않아 제외되었습니다.</p>
        
        <details>
          <summary>후보 벨 소리 정보 (${debug.candidateBells.length}개)</summary>
          <ul class="debug-list">
            ${debug.candidateBells.map(bell => 
              `<li>시작: ${formatTime(bell.start)}, 진폭: ${bell.peakAmplitude.toFixed(3)}, 길이: ${bell.duration.toFixed(2)}초
                <button class="btn-small btn-preview" data-time="${bell.start}">미리보기</button>
              </li>`
            ).join('')}
          </ul>
        </details>
        ...
        <details>
          <summary>거부된 벨 소리 정보 (${debug.rejectedBells.length}개)</summary>
          <ul class="debug-list">
            ${debug.rejectedBells.map(bell => 
              `<li>시작: ${formatTime(bell.start)}, 진폭: ${bell.peakAmplitude.toFixed(3)}, 길이: ${bell.duration.toFixed(2)}초, 거부 이유: ${bell.reason}
                <button class="btn-small btn-preview" data-time="${bell.start}">미리보기</button>
              </li>`
            ).join('')}
          </ul>
  ...
  if (suggestButton && debug && debug.candidateBells.length > 0) {
    suggestButton.addEventListener('click', () => {
      ...
      alert(`임계값이 ${suggestedThreshold}로 조정되었습니다. '벨 소리 감지' 버튼을 다시 클릭하세요.`);
    });
  }
  ...
  // 마커 직접 클릭 이벤트
  const allMarkers = document.querySelectorAll('.bell-marker, .bell-candidate, .bell-rejected');
...
function setupTimelineInteractions() {
  ...
  timelineEl.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('bell-marker') || 
...
function updatePlayerPosition() {
  ...
  bellMarkers.forEach(marker => {
    ...
    if (Math.abs(currentTime - markerTime) < 0.5) { // 0.5초 이내면 강조
      marker.classList.add('marker-active');
    } else {
      marker.classList.remove('marker-active');
    }
  ...
  timestampItems.forEach(item => {
    ...
    if (Math.abs(currentTime - itemTime) < 0.5) { // 0.5초 이내면 강조
...
function formatTime(seconds) {
  ...
  if (min >= 60) {
    ...
    return `${String(hour).padStart(2, '0')}:${String(remainMin).padStart(2, '0')}:${String(sec).padStart(2, '0')}`;
  }
